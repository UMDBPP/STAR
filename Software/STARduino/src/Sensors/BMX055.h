...
//Converstion factors
#define BMX_ACCEL_TEMP_CONVERSTION_FACTOR .5f //.5K / LSB
...
#define BMX_GYRO_CONVERSTION_FACTOR .061f //degrees/sec, based on the decimal value in the register
...
class BMX055_ACCEL {
	public:
		...
		Vector3<uint16_t> get_offset() {return offsetAccel;}
		Vector3<uint16_t> get_accel() {return rawAccel;}

	private:
		Vector3<uint16_t> rawAccel;
		uint8_t numSamples;
		Vector3<uint16_t> offsetAccel;
};	
...
class BMX055_GYRO {
	public:
		...
		uint16_t get_x_gyro() {return rawGyro.x;}
		uint16_t get_y_gyro() {return rawGyro.y;}
		uint16_t get_z_gyro() {return rawGyro.z;}
		Vector3<uint16_t> get_gyro() {return rawGyro;}

	private:
		Vector3<uint16_t> rawGyro;
		uint8_t numSamples;
};
...
class BMX055_MAG {
	public:
		...
		uint16_t get_x_mag() {return rawMag.x;}
		uint16_t get_y_mag() {return rawMag.y;}
		uint16_t get_z_mag() {return rawMag.z;}
		Vector3<uint16_t> get_mag() {return rawMag;}

	private:
		Vector3<uint16_t> rawMag;
		uint8_t numSamples;
		uint16_t hall
};
#endif //BMX_H